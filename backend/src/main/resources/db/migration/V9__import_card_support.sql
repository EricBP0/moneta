...
ALTER TABLE import_row
  ...
  ADD COLUMN resolved_account_id BIGINT REFERENCES account(id) ON DELETE SET NULL,
...
-- Create indexes for account and card references
CREATE INDEX idx_import_row_resolved_account_id ON import_row (resolved_account_id);
